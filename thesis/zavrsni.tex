...
Pogledajmo jednu jednostavnu unaprijednu neuronsku mrežu s arhitekturom 2x3x2.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{img/ffann.png}
...
\begin{equation}
    ...
    \text{$\Vec{net}\textsuperscript{(l+1)} = W\textsuperscript{(l)} * \Vec{y}\textsuperscript{ (l)} + \Vec{b}\textsuperscript{ (l)}$}
\end{equation}

\begin{equation}
    \label{eq:izlaz-matrice}
    \text{$\Vec{y}\textsuperscript{ (l+1)} = g(W\textsuperscript{(l)} * \Vec{y}\textsuperscript{ (l)} + \Vec{b}\textsuperscript{ (l)})$}
\end{equation}
...
\[
  ...
  \begin{bmatrix}
   y\textsubscript{1}\textsuperscript{(1)} && y\textsubscript{2}\textsuperscript{(1)}
  \end{bmatrix},\quad
