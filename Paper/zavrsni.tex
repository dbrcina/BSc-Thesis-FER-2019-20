...
\textit{Polinomijalna regresija} \engl{Polynomial regression} je tip regresije koji koristi polinom reda \textit{r} kao aproksimacijsku krivulju.
...
\end{figure}
\footnotetext{Izrađeno pomoću programa PowerPoint i na temelju modela umjetnog neurona iz literature \citep{cupicANN}.}
...
\begin{figure}[H]
    ...
    \label{fig:ann}
\end{figure}
\footnotetext{Izrađeno pomoću programa PowerPoint i na temelju modela umjetne neuronske mreže iz literature \citep{cupicNENR}.}
...
\end{figure}
\footnotetext{Izrađeno pomoću programa PowerPoint i na temelju modela umjetne neuronske mreže iz literature \citep{cupicANN}.}
...
\begin{figure}[H]
    ...
    \label{fig:ffann}
\end{figure}
\footnotetext{Izrađeno pomoću programa PowerPoint i na temelju modela umjetne neuronske mreže iz literature \citep{nielsenANN}.}
...
\begin{enumerate}
    ...
    \end{figure}
    \footnotetext{Preuzeto iz literature \citep{cupicANN}.}
    \item \textbf{Funkcija zglobnica} \engl{Rectified Linear Unit, ReLU} (prikazana na slici \ref{fig:relu-fun}):
    \begin{equation}
        \label{eq:relu}
        relu(net) = max(0, net),
    \end{equation}
    ...
    \end{figure}
    \footnotetext{Preuzeto iz literature \citep{cupicANN}.}
    \item \textbf{Funkcija \textit{softmax}}: \newline
    Funkcija \textit{softmax} je funkcija koja ulaze transformira na vrijednost između 0 i 1 baš kao i sigmoidalna funkcija pa je graf praktički identičan te ga nema smisla ponovno prikazivati. No, ideja \textit{softmax} funkcije je da vrati postotke u kojima se dotični ulazi nalaze i onda suma svih tih postotaka mora biti stopostotna. Koristi se samo kao aktivacijska funkcija izlaznog sloja i vrlo je pogodna kada se radi o problemima višerazredne klasifikacije jer je onda preko postotaka lako za odrediti koji razred se nalazi na izlazu. Definicija funkcije je malo drugačija od ostalih zato što kao ulazni argument prima vektor vrijednosti, a ne samo jednu vrijednost i glasi ovako \citep{\textit{softmax}}:
    \[
        S(\vec{a}) : \quad
        \begin{bmatrix}
            a\textsubscript{1} \\
...
\begin{enumerate}
    ...
    \item Stohastičko učenje \engl{Stochastic learning} - težine se ažuriraju nakon svakog uzorka, vremenski je najpovoljnije, ali je nešto neprecizniji od učenja po manjim grupama. Često se naziva i \textit{online} učenje zbog ažuriranja nakon svakog uzorka.
\end{enumerate}
